...
- **Integrated Kittelsen & Larvikite Theme Suites**:
  - Integrated 8 built-in themes into default configuration, backend theme provider (`src/config/mod.rs`), `config.toml`, and frontend palette:
    - **Larvikite Series**: `skumring` (Larvikite Skumring), `demring` (Larvikite Demring), `trollnatt` (Larvikite Trollnatt), and `myrtaake` (Larvikite Myrtåke).
    - **Kittelsen Series**: `bergtatt` (Kittelsen Bergtatt), `soria-moria` (Kittelsen Soria Moria), `pestanatt` (Kittelsen Pestanatt), and `sotslette` (Kittelsen Sotslette).
  - Added visual swatch cards, settings dropdown options, and Spotlight command palette switchers (`Ctrl+K`) for all 8 themes.
  - Added standalone `themes/kittelsen.toml` theme bundle for external directory discovery.
