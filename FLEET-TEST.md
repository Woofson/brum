# Commander Fleet: Testing & Verification Guide

Welcome to the **Commander Fleet** verification walkthrough. This step-by-step guide will walk you through testing multi-node orchestration, cross-pane remote file management, and streamed cross-node transfers in Brum.
...
2. Open **Settings** (<kbd>F10</kbd>).
3. Click the **Security & Access** tab, then click **API Tokens & Service Credentials**.
...
## Step 3: Register Node 2 in Fleet Manager on Node 1

1. Return to your primary Brum session: `http://localhost:3140`.
2. Open the **Tools Launchpad** $\rightarrow$ select **Fleet Manager** (or click the Server badge in the top header $\rightarrow$ **Fleet Manager**).
...
3. Press <kbd>F7</kbd> (or click `+ New Folder`) in Panel 2:
...
## Step 7: Test Remote Viewers, Code Editor & Media Streaming
...
2. **Code Editor (`F4`)**:
   - Select a remote file in Panel 2 and press <kbd>F4</kbd>.
   - Make an edit and press <kbd>Ctrl+S</kbd>.
   - Verify the file is saved directly to the remote node.
3. **Audio Streaming**:
   - Double click an `.mp3` or `.flac` file in Panel 2 (Remote).
   - Verify Audio Player streams audio smoothly from the remote host with seeking and ID3 tags.
4. **Video Player**:
